...
songs = Song.objects.raw('SELECT api_song.* FROM api_song limit 6000')
...
def songSearch(request):
    ...
    songs = Song.objects.raw(
        'SELECT * FROM api_song WHERE api_song.song_id < 6000 AND song_title REGEXP %s or song_artist REGEXP %s LIMIT 20', [search, search])
    serializer = SongSerializer(songs, many=True)
    return Response(serializer.data, status.HTTP_200_OK)
